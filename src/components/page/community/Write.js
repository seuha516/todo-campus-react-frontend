...
import { initWritePostError, writePost } from "modules/post";
...
const Write = () => {
    const { username, nickname, loading, error } = useSelector(
        ({ account, loading, post }) => ({
            username: account.user.username,
            nickname: account.user.nickname,
            loading: loading["post/WRITE_POST"],
            error: post.writeError,
        })
    );
    ...
    const onSubmit = () => {
        ...
        dispatch(writePost(submitValue));
    };
        ...
        <Wrapper>
                    ...
                    <AddImageButton
                        onClick={() => {
                            if (state.image.length >= 5) {
                                ...
                                document
                                    .getElementById("FileInput_CommunityWrite")
                                    .click();
                            }
                        }}
                    >
            ...
            <TagWrapper>
                ...
                <TagForm onSubmit={addTag}>
                    ...
                    <TagInput id="TagInput_CommunityWrite" placeholder="태그" />
